...
def wa_cleanup_sessions() -> tuple[bool, str]:
    ...
    try:
        ...
        # fallback para /logout se /cleanup não existir
...
async def cb_wa_logs(cq: CallbackQuery):
    ...
    for l in logs:
        ok = "✅" if l["ok"] else "❌"
        when = l["created_at"].astimezone(ZoneInfo(TZ_NAME)).strftime("%d/%m %H:%M")
...
async def cb_sched_toggle(cq: CallbackQuery):
    ...
    # atualiza teclado principal do menu WA
...
async def send_due_messages():
    """Envia mensagens automáticas para D-2, D-1, D0 e D+1."""
    ...
    for c in clients:
        ...
        if key and key in valid_keys:
            ...
            add_wa_log(c.get("id"), phone, text, ok, msg)

async def scheduler_loop():
    while True:
        try:
            if not _sched_enabled():
                await asyncio.sleep(30)  # checa periodicamente se foi ligado
                continue
            ...
            print("[SCHED] Executando envio automático…")
            await send_due_messages()
            # aguarda 60s para evitar rodar duas vezes no mesmo minuto

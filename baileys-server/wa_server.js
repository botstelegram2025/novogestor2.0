...
async function ensureJobsTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS wa_jobs (
      ...
      status TEXT NOT NULL DEFAULT 'queued',  -- queued | processing | sent | failed | canceled
      error TEXT,
...
async function startBaileys() {
  ...
  const logger = pino({ level: 'fatal' }); // menos ruído de log
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // Reconecta automaticamente exceto se foi logout explícito
...
// Agenda no banco para o worker enviar no horário
...
// Limpa sessões: faz logout e apaga credenciais
...
// Busca jobs vencidos e envia pelo Baileys.
async function runSchedulerTick() {
  try {
    // pega até 10 jobs "queued" prontos
    ...
    for (const job of jobs) {
      // marca como processing (evita disputar)
      const upd = await pool.query(
        `UPDATE wa_jobs SET status='processing' WHERE id=$1 AND status='queued'`,
        [job.id]
      );
      if (upd.rowCount === 0) continue; // outro worker pegou
      ...
      if (ok) {
        await pool.query(
          `UPDATE wa_jobs SET status='sent', sent_at=NOW(), error=NULL WHERE id=$1`,
          [job.id]
        );
      } else {
        await pool.query(
          `UPDATE wa_jobs SET status='failed', error=$2 WHERE id=$1`,
          [job.id, err]
        );
      }
    }
